...
def main():
    ...
    tmx_grid = build_tiled_map(grid)
    save_tmx_file(tmx_grid, 'test_world.tmx')
...
def build_tiled_map(grid):
  ...
  for row in grid:
    ...
    for col in row:
      cur_line.append(get_tile_index(col))
...
def get_tile_index(letter):
    """
    lookups up the letter in the tileset definition
    and returns the grid index (single value for TMX)
    """
    if letter in tile_indexes:
        return tile_indexes[letter]
    else:
        return -1


def save_tmx_file(tmx_grid, op_file):
    """
    ...
    with open(op_file, 'w') as f:
        ...
        f.write(' <tileset firstgid="1" source="mountain_landscape.tsx"/>\n')
